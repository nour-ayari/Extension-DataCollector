...
const TN_CITIES = [
  { city: 'Tunis',       zip: '1000' }, { city: 'Sfax',        zip: '3000' },
  { city: 'Sousse',      zip: '4000' }, { city: 'Ettadhamen',  zip: '2041' },
  { city: 'Kairouan',    zip: '3100' }, { city: 'Bizerte',     zip: '7000' },
  { city: 'Gabès',       zip: '6000' }, { city: 'Ariana',      zip: '2080' },
  { city: 'Gafsa',       zip: '2100' }, { city: 'Monastir',    zip: '5000' },
  { city: 'Ben Arous',   zip: '2013' }, { city: 'Kasserine',   zip: '1200' },
  { city: 'Médenine',    zip: '4100' }, { city: 'Nabeul',      zip: '8000' },
  { city: 'Tataouine',   zip: '3200' }, { city: 'Béja',        zip: '9000' },
  { city: 'Jendouba',    zip: '8100' }, { city: 'El Kef',      zip: '7100' },
  { city: 'Mahdia',      zip: '5100' }, { city: 'Sidi Bouzid', zip: '9100' },
  { city: 'Tozeur',      zip: '2200' }, { city: 'Kebili',      zip: '4200' },
  { city: 'Zaghouan',    zip: '1100' }, { city: 'Siliana',     zip: '6100' },
  { city: 'Manouba',     zip: '2010' }, { city: 'La Marsa',    zip: '2070' },
  { city: 'Hammam-Lif',  zip: '2050' }, { city: 'Djerba',      zip: '4180' },
  { city: 'Zarzis',      zip: '4170' }, { city: 'Hammamet',    zip: '8050' },
];
...
function randomTunisianAddress() {
  ...
  return {
    street:  `${number} ${faker.helpers.arrayElement(TN_STREET_TYPES)} ${faker.helpers.arrayElement(TN_STREET_NAMES)}`,
    city, zip, country: 'TN',
  };
}
...
function buildOrderPayload(cartItems, user, probs, orderTs) {
  ...
  return {
    ...
    order_description: {
      ...
      promo_code:            usedPromo ? `PROMO${faker.string.alphanumeric(5).toUpperCase()}` : null,
      items_count:           cartItems.length,
      unique_products_count: new Set(cartItems.map(i => i.product_id)).size,
      categories:            [...new Set(cartItems.map(i => i.category))],
      order_lines:           cartItems.map(i => ({
        product_id: i.product_id, product_name: i.product_name, category: i.category,
        price: i.price, quantity: i.quantity, subtotal: parseFloat((i.price * i.quantity).toFixed(2)),
      })),
      ...
      customer_snapshot: {
        age: user.age, gender: user.gender, persona: user.persona, nb_visits: user.nb_visits,
      },
    },
  };
}
...
async function simulateSession(site, user) {
  ...
  await identify(user.anonymousId, {
    ...
    persona: user.persona, loggedIn, nb_visits: user.nb_visits, device,
  }, sessionTs);
  ...
  if (roll(probs.bounce)) {
    ...
    await track('page_engagement', {
      ...base('home', site),
      duration: bounceSec, max_scroll_pct: faker.number.int({ min: 0, max: 25 }),
      click_count: 0, is_bounce: true, cart_abandoned: false,
      sequence_summary: sequence.join(' → '), order_description: null, orders: 0,
  ...
  if (roll(probs.promoPage)) {
    ...
    await track('page_view',    { ...base('promo', promoUrl) }, user.anonymousId, tsOffset(sessionTs, ms));
    await track('promo_viewed', { ...base('promo', promoUrl), promo_url: promoUrl }, user.anonymousId, tsOffset(sessionTs, ms));
    sequence.push('promo');
  ...
  for (let i = 0; i < probs.nbProducts; i++) {
    ...
    const product    = { product_id: faker.string.uuid(), product_name: faker.commerce.productName(), price, category };
    const productUrl = `${site}/product/${faker.string.alphanumeric(8)}`;

    await track('product_view', { ...base('product', productUrl), ...product }, user.anonymousId, tsOffset(sessionTs, ms));
    sequence.push('product_view');
    ms += probs.thinkProduct;

    await track('scroll_depth', {
      ...base('product', productUrl), depth_pct: probs.scrollDepth,
    }, user.anonymousId, tsOffset(sessionTs, ms));

    if (roll(probs.addToCart)) {
      const qty = probs.qty;
      cartItems.push({ ...product, quantity: qty });
      await track('add_to_cart', { ...base('product', productUrl), ...product, quantity: qty }, user.anonymousId, tsOffset(sessionTs, ms));
      sequence.push('add_to_cart');
      ms += faker.number.int({ min: 300, max: 1200 });

      if (roll(probs.removeFromCart) && cartItems.length > 0) {
        cartItems.pop();
        await track('remove_from_cart', { ...base('cart', `${site}/cart`), ...product, quantity: qty }, user.anonymousId, tsOffset(sessionTs, ms));
    ...
    if (i < probs.nbProducts - 1 && roll(probs.goBack)) {
      await track('page_view', { ...base('category', `${site}/category/${category}`) }, user.anonymousId, tsOffset(sessionTs, ms));
      ms += faker.number.int({ min: 400, max: 1500 });
    }
  }

  if (roll(probs.browseCart) && cartItems.length === 0) {
    await track('page_view', { ...base('cart', `${site}/cart`) }, user.anonymousId, tsOffset(sessionTs, ms));
  ...
  if (cartItems.length > 0) {
    ...
    if (roll(probs.cartToCheckout)) {
      ...
      await track('checkout_started', {
        ...base('checkout', `${site}/checkout`), ...checkoutPayload,
        checkout_step_reached: faker.helpers.arrayElement(['address', 'shipping', 'payment']),
        order_status: 'pending', payment_status: 'pending', recovery_email_eligible: false,
      }, user.anonymousId, tsOffset(sessionTs, ms));
      ...
      if (roll(probs.checkoutToPurchase)) {
        ...
        await track('purchase_completed', {
          ...base('checkout', `${site}/order-confirm`), ...purchasePayload,
        ...
        await track('checkout_abandon', {
          ...base('checkout', `${site}/checkout`), ...abandonPayload,
          order_status: 'abandoned', payment_status: 'pending',
          checkout_step_reached:   faker.helpers.arrayElement(['address', 'shipping', 'payment']),
      ...
      await track('cart_abandon', {
        ...base('cart', `${site}/cart`), ...abandonPayload,
        order_status: 'abandoned', payment_status: 'none',
